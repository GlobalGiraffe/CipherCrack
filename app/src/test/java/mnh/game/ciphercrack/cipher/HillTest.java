...
public class HillTest {
    ...
    public void testBadParameters() {
        ...
        reason = cipher.canParametersBeSet(p);
        assertEquals("BadParam: missing matrix", "Matrix is not valid", reason);
        p.setMatrix(new int[0]);
        reason = cipher.canParametersBeSet(p);
        assertEquals("BadParam: empty matrix", "Matrix is not valid", reason);
        p.setMatrix(new int[] {0,2,1,2,9}); // matrix not square
        reason = cipher.canParametersBeSet(p);
        p.setMatrix(new int[] {0,2,1,2,9,8,3,1,12,23,1,0,5,8,19,15}); // can't deal with 4x4 or bigger yet
        reason = cipher.canParametersBeSet(p);
        assertEquals("BadParam: extra entry", "Cannot invert 4x4 or higher matrices yet", reason);
        p.setMatrix(new int[] {3,1,-2,24});  // negative number
        reason = cipher.canParametersBeSet(p);
        assertEquals("BadParam: too big #1", "Matrix element -2 is negative", reason);

        p.setMatrix(new int[] {23,1,12,0,0,1,18,3,29});  // 29 is too large
        reason = cipher.canParametersBeSet(p);
        assertEquals("BadParam: too big #2", "Matrix element 29 is too large", reason);

        p.setMatrix(new int[] {2, 0, 0, 0});
        reason = cipher.canParametersBeSet(p);
        assertEquals("BadParam: not co-prime", "Determinant is 0, matrix is singular and cannot decode", reason);

        p.setMatrix(new int[] {1, 0, 0, 13});
        reason = cipher.canParametersBeSet(p);
        assertEquals("BadParam: not co-prime", "Matrix determinant is 13, cannot decode uniquely", reason);

        p.setMatrix(new int[] {2,3,1,0});
    ...
    // this one can take around 1 min 16 secs to find 7,8,11,11
    @Test
    public void testCrackBruteForceCipherChallenge() {
        ...
        int[] decodeMatrix = result.getDirectives().getMatrix();
    ...
    public void testCrackDictReverseSuccess() {
        ...
        p.setMatrix(encodeMatrix);
        String reason = cipher.canParametersBeSet(p);
        assertNull("CrackDictReverse: encode param okay", reason);
        String cipherText = cipher.encode(new StringBuilder(plainText).reverse().toString(), p);
        assertNotNull("CrackDictReverse: Encoding", cipherText);
        ...
        reason = cipher.canParametersBeSet(p);
        assertNull("CrackDictReverse: crack param okay", reason);
        ...
        int[] decodeMatrix = result.getDirectives().getMatrix();
        System.out.println("Matrix "+Hill.matrixToString(decodeMatrix));
        assertTrue("CrackDictReverse success", result.isSuccess());
        assertEquals("CrackDictReverse Cipher", cipherText, result.getCipherText());
        assertEquals("CrackDictReverse Text", "X"+plainText.replaceAll("\\W","").toUpperCase(), result.getPlainText());
        assertEquals("CrackDictReverse Keyword", keyword, decodeKeyword);
        assertEquals("CrackDictReverse Matrix", expectedMatrix, Hill.matrixToString(decodeMatrix));
        assertNotNull("CrackDictReverse Explain", explain);
        assertTrue("CrackDictReverse Explain REVERSE", explain.contains("REVERSE"));
        assertTrue("CrackDictReverse Explain start", explain.startsWith("Success"));
        assertEquals("CrackDictReverse cipher name", "Hill cipher (7,8,11,11)", result.getCipher().getInstanceDescription());
    }
